...
export default function App() {
  ...
  /**
   * Runs `op(token)`. If it throws UnauthorizedError, silently refreshes
   * the token once and retries. Signs out only if the refresh itself fails.
   */
  const withRefresh = useCallback(
    async <T,>(op: (token: string) => Promise<T>): Promise<T> => {
      ...
      try {
        ...
        const newToken = await silentRefresh(); // throws if refresh fails
    ...
    async (t: string) => {
      ...
      try {
        const runLoad = async (token: string) => {
          ...
          setUser(profile);
          const [evs, armedIds, line, callLogs] = await Promise.all([
        ...
        if (e instanceof UnauthorizedError) {
          // Silent refresh also failed — force sign-in
          signOut();
  ...
  return (
    ...
    >
      ...
      {view === 'overview' && (
        <Overview events={events} armed={armed} settings={settings} logs={logs} onNavigate={setView} />
      )}
      {view === 'calendar' && (
        <CalendarView
          events={events}
